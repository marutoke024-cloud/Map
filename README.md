...
| 要件 | 実装 |
|---|---|
| 階層ナビ（日本 → 近畿/関東 → 府県） | `currentView` ステート + GSAP フライズーム (`src/main.js`, `src/map/mapRenderer.js`) |
| 図形のみ・ラベル無しの日本地図 | TopoJSON を `d3-geo` で投影、近畿/関東のみ発光・タップ可 |
| 1段ずつ戻る | BACK ボタン / パンくず / Esc キー |
| ピン設置・詳細パネル | 府県ビューで地図タップ → パネル (`src/pins/pinPanel.js`) |
